...
describe('convolution module registry and URL params', () => {
  it('registers the convolution module once with all six ready routes', () => {
    ...
    expect(modules[0].explorers).toHaveLength(6)
    expect(resolveRoute('/modules/convolution').kind).toBe('module')
    expect(resolveRoute('/modules/convolution/discrete').kind).toBe('module')
    expect(resolveRoute('/modules/convolution/probability').kind).toBe('module')
    expect(resolveRoute('/modules/convolution/signal').kind).toBe('module')
    expect(resolveRoute('/modules/convolution/image-kernel').kind).toBe('module')
    expect(resolveRoute('/modules/convolution/polynomial').kind).toBe('module')
    expect(resolveRoute('/modules/convolution/continuous').kind).toBe('module')
  ...
  it('reads explorer URL params and falls back on invalid params', () => {
    expect(decodeDiscreteState(new URLSearchParams('a=1,2&b=3&convMode=same&operation=correlation&k=1')).mode).toBe('same')
    expect(decodeProbabilityState(new URLSearchParams('preset=d6-d6&sum=7')).sum).toBe(7)
    expect(decodeSignalState(new URLSearchParams('signal=sine&kernel=gaussian&convMode=same&boundary=wrap&length=64&sigma=1.2&noise=0.2&seed=4&index=3')).boundary).toBe('wrap')
    ...
    expect(decodeDiscreteState(new URLSearchParams('convMode=bad&a=')).mode).toBe('full')
