...
export const appCopy: Record<Locale, AppCopy> = {
  en: {
    ...
    matrixSequence: {
      ...
      add: 'Add',
      help: 'User order [A, B, C] means first A, then B, then C. In multiplication the vector sits on the right, so the composed matrix is CBA.',
  ...
  zh: {
    ...
    matrixSequence: {
      title: '矩阵序列',
      add: '添加',
      help: '用户顺序 [A, B, C] 表示先作用 A，再作用 B，再作用 C。写乘法时向量在最右边，所以合成矩阵是 CBA。',
      presetTitle: '预设库',
